...
class EquilibriumMatching :
    ...
    """Equilibrium Matching : https://arxiv.org/pdf/2510.02300
    """
    
    def __init__(self, sched='linear', lamda = 4, **kwargs):
        
        self.lamda = lamda
        if sched =="linear":
            self.c = lambda t : 1 - t
        elif sched=="trunc_decay":
            assert "a" in kwargs.keys(), "missing parameter for truncated decay"
            self.c = lambda t, a=kwargs["a"] : torch.where(t <= a, 1., (1 - t)/(1 - a)) # 1 if t <= a else (1 - t)/(1 - a) 
        elif sched=="piecewise":
            assert "a" in kwargs.keys() and "b" in kwargs.keys(),  "missing parameter(s) for piecewise"
            self.c = lambda t, a=kwargs["a"], b=kwargs["b"] : b - (b - 1)/a * t if t <= a else (1 - t)/(1 - a)
        else:

...
def infer_stitched(len_of_traj, start, end, dims_of_img, total_samples, samples_per_batch,
                ...
                y_list = None, y0_provided = False, y0= None, all_traj=False, w=1, **kwargs):
            ...
            for t in time:
                for index, x_ind in enumerate(x):
                    ...
                    if y_list is not None:
                        y = torch.tensor(y_list[index]).to(device)[None]
                        v = w * cfm_model(t, x_inp, y) + (1 - w) * cfm_model(t, x_inp, None)
                    else:
                        y = None
                        v = cfm_model(t, x_inp, y)
                    x_ind = x_ind + (v*dt)[0] 
                    x[index] = x_ind
...
def infer_stitched_2(len_of_traj, start, end, dims_of_img, total_samples, samples_per_batch,
                cfm_model, t_start, t_end,
                scale, device, m=None, std=None, t_steps=100, use_heavy_noise=False, 
                y_list = None, y0_provided = False, y0= None, all_traj=False, w=1, **kwargs):
        ...
        with torch.no_grad():
            ...
            for t in time:
                for index, x_ind in enumerate(x):
                    ...
                    # if index !=0:
                    #     x[index - 1] = (x_inp[0, 0:1] + x[index - 1])/2.
                    # if index != len_of_traj -1:
                    #     x[index + 1] = (x_inp[0, 2:3] + x[index + 1])/2.
...
def infer_auto_reg(len_of_traj, start, dims_of_img, total_samples, samples_per_batch,
                cfm_model, t_start, t_end,
                scale, device, m=None, std=None, t_steps=100, use_heavy_noise=False, 
                y_list = None, y0_provided = False, y0= None, all_traj=False, w=1, **kwargs):
        ...
        with torch.no_grad():
            ...
            for t in time:
                for index, x_ind in enumerate(x):
                    ...
                    if index == 0:
                        x_inp = torch.cat([start, x_ind], dim=0)[None]
                    else:
                        x_inp = torch.cat([x[index -1], x_ind], dim=0)[None]

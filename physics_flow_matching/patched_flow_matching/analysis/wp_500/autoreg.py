...
# %%
total_samples = 100
stream_wise_length = 2400
small_domain_length = 599
window_size = 598
...
for _ in range(total_samples):
    ...
    for i in range(stream_wise_length//window_size):
        if i == 0:
            samples = infer_patched(dims_of_img=(1,small_domain_length,399), patch_size=(56,56), total_samples=1, samples_per_batch=1, cfm_model=model, t_start=0, t_end=1, scale=False, t_steps=200, device=device, ignore_index=1,  inner_scale=0.4920473773265651)
        else:
            measurements = torch.from_numpy(long_sample[-1][..., window_size:, :]).to(device)
            samples = flow_padis(FlowMatcher(1e-3), model, samples_per_batch=1, total_samples=1, dims_of_img=(1,small_domain_length,399), patch_size=(56, 56), num_of_steps=200, grad_cost_func=grad_cost_func,
                            ...
                            rf_start=False, swag=False, nu=3,  sx=0, ex=small_domain_length-window_size, sy=0, ey=400, ignore_index=1,  inner_scale=0.4920473773265651)
        long_sample.append(samples)
    long = np.concat([gen if i==0 else  gen[..., small_domain_length-window_size:, :] for i, gen in enumerate(long_sample)], axis=2)
...
# %%
np.save(f"/home/meet/FlowMatchingTests/conditional-flow-matching/physics_flow_matching/patched_flow_matching/exps/wp/exps_length_inner_scaled/exp_{exp}/samples_uncond_autoreg_{iteration}.npy", long_samples_*std + m)
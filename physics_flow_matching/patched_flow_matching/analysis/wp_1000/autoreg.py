...
# %%
total_samples = 100
stream_wise_length = 4800
small_domain_length = 1199
window_size = 1198
...
for _ in range(total_samples):
    ...
    for i in range(stream_wise_length//window_size):
        if i == 0:
            samples = infer_patched(dims_of_img=(1,small_domain_length,799), patch_size=(56,56), total_samples=1, samples_per_batch=1, cfm_model=model, t_start=0, t_end=1, scale=False, t_steps=200, device=device, ignore_index=1, 
                                    inner_scale=1.0)
        else:
            measurements = torch.from_numpy(long_sample[-1][..., window_size:, :]).to(device)
            samples = flow_padis(FlowMatcher(1e-3), model, samples_per_batch=1, total_samples=1, dims_of_img=(1,small_domain_length,799), patch_size=(56, 56), num_of_steps=200, grad_cost_func=grad_cost_func,
                            ...
                            rf_start=False, swag=False, nu=3,  sx=0, ex=small_domain_length-window_size, sy=0, ey=800, ignore_index=1,  inner_scale=1.)
